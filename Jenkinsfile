pipeline {
  ...
  stages {
    ...
    stage('DeploymentToSandbox') {
      ...
      steps {
            bat 'mvn -U -V -e -B -DskipTests -PSandbox deploy -DmuleDeploy  -Dusername="%ANYPOINT_USR%" -Dpassword="%ANYPOINT_PSW%" -Dsandbox.anypoint.platform.client_id="%sandbox_client_id%" -Dsandbox.anypoint.platform.client_secret="%sandbox_client_secret%"'
      }  
    }
  }
}